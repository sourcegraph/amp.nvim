...
function M.has_selection_changed(new_selection)
	...
	if
		old_sel.start.line ~= new_sel.start.line
		or old_sel.start.character ~= new_sel.start.character
		or old_sel["end"].line ~= new_sel["end"].line
		or old_sel["end"].character ~= new_sel["end"].character
	then
		return true
	end
