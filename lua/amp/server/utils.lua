...
---@param path string The URL path (e.g., "/websocket?auth=token&other=value")
---@param param_name string The parameter name to extract
---@return string|nil value The parameter value if found, nil otherwise
function M.extract_query_param(path, param_name)
	if not path or not param_name then
		return nil
	end
	...
	-- Handle both "&param=value" and "param=value" (at start)
	local value = query_string:match("^" .. param_name .. "=([^&]*)") -- At start
	if not value then
		value = query_string:match("&" .. param_name .. "=([^&]*)") -- After &
	end

	if value then
		-- URL decode the value (basic version - handles %XX encoding)
